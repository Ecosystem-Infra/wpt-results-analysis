...
/**
 * Implements a view of WPT pass rate per browser over time.
 */
...
flags.defineStringList('products', ['chrome', 'firefox', 'safari'],
    'Browsers to report for. Must match the products used on wpt.fyi');
flags.defineString('output', null,
    'Output CSV file to write to. Defaults to ' +
    '{stable, experimental}-browser-pass-rate.csv');
...
// inclusive, exclusive.
// TODO: maybe remove these for this analysis.
...
//
// TODO: Maybe dont need to be aligned for this analysis
...
async function main() {
  ...
  console.log('Calculating pass rate for the runs');
  before = Date.now();
  ...
  for (const [date, runs] of alignedRuns.entries()) {
    ...
    try {
      const scores = lib.browserPassRate.scoreBrowserPassRates(
  ...
  if (!outputFilename) {
    outputFilename = experimental ?
        'experimental-browser-pass-rate.csv' :
        'stable-browser-pass-rate.csv';
  }
