...
/* Based on browser-specific-failures.js, with some features removed */
...
async function main() {
  ...
  console.log('Calculating css/css-flexbox scores for the runs');
  before = Date.now();
    ...
    try {
      for (const run of runs) {
        ...
        lib.results.walkTests(run.tree, (path, test, results) => {
          if (!path.startsWith('/css/css-flexbox') ||
              path.startsWith('/css/css-flexbox/animation')) {
            return;
          }
          totalTests++;
          if ('subtests' in results) {
            ...
            passes += subtestPasses / results['subtests'].length;
          } else if (results['status'] == 'PASS') {
            passes += 1;
          }
        });
        scores.push(passes / totalTests);
      }
  ...
  if (!outputFilename) {
    outputFilename = experimental ?
        'experimental-css-flexbox-scores.csv' :
        'stable-css-flexbox-scores.csv';
  }
