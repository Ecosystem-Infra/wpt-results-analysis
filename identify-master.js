...
async function main() {
  ...
  for (const line of lines) {
    ...
    if (!run.startTime.startsWith('2018')) {
      continue;
    }
  ...
  // During 2018, the number of tests grows from about 24k in January to 27.5k
  // in June. We use that to map to our expectations for tests.
  const expected_insertions = new Map([
      [ '2018-01', 24000 ],
      [ '2018-02', 25000 ],
      [ '2018-03', 25000 ],
      [ '2018-04', 25500 ],
  ...
  for (const run of runs) {
    ...
    const month = run.startTime.substring(0, 7);
    if (!expected_insertions.has(month)) {
      continue;
    }

    if (run.insertions >= expected_insertions.get(month)) {
  ...
  let output = 'should_be_master_run_ids_2018.txt';
