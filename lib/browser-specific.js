...
const SUBTEST_FAIL_STATUSES = ['FAIL', 'ERROR', 'TIMEOUT'];
const SUBTEST_NEUTRAL_STATUSES = ['PRECONDITION_FAILED', 'SKIP', 'NOTRUN'];
...
function findSmallestNameAndIndex(browserSubtests) {
  let smallestName = browserSubtests[0].value().name;
  let smallestIdx = 0;
  for (let i = 1; i < browserSubtests.length; i++) {
    if (browserSubtests[i].value().name < smallestName) {
