...
// enumerable properties, or an Array. Iterates in a sorted order; alphabetical
// in the case of an Object, or in the order defined by |comparatorFunc| in the
// case of an Array.
class IteratorHelper {
  constructor(arrOrObject, comparatorFunc = null) {
    this.currentIndex = 0;
...
function scoreTest(browserTests, scores) {
  // Some WPT tests contain multiple 'subtests' (e.g. most jsharness tests),
  // whilst others are just a single conceptual test (e.g. reftests).
  //
  ...
  if (browserTests.every(tests => !tests.subtests || tests.subtests.length == 0)) {
    let failed = [];
    ...
    const comparator = (s1, s2) => (s1.name > s2.name) ? 1 : -1;
    ...
    let denominator = 0;
    let counts = new Array(browserSubtests.length).fill(0);
...
function walkTrees(browserTrees, scores) {
  ...
  const browserTests = browserTrees.map(tree => new IteratorHelper(tree.tests));
  while (browserTests.every(tests => tests.hasNext())) {
    ...
    if (browserTests.every(t => t.currentValue() == firstTests.currentValue())) {
  ...
  const browserSubtrees = browserTrees.map(tree => new IteratorHelper(tree.trees));
  while (browserSubtrees.every(subtree => subtree.hasNext())) {
    ...
    if (browserSubtrees.every(s => s.currentValue() == firstTree.currentValue())) {
...
function scoreBrowserSpecificFailures(runs, expectedBrowsers) {
  ...
  if (seenBrowsers.size != expectedBrowsers.size) {
    let difference = new Set([...expectedBrowsers].filter(x => !seenBrowsers.has(x)));
    throw new Error(`Run has missing browsers: ${[...difference].join(',')}`);
  }
