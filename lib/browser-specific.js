...
const KNOWN_PREFIXES = new Set(['/css']);
function isTopLevel(path) {
  if (KNOWN_PREFIXES.has(path))
    return false;
  ...
  for (const prefix of KNOWN_PREFIXES.values()) {
    const numPrefixSlashes = (prefix.match(/\//g)||[]).length;
    if (path.startsWith(prefix + '/') && numSlashes == numPrefixSlashes + 1)
      return true;
  }
...
function walkTrees(browserTrees, path, perDirScores, testFilter) {
  ...
  while (browserSubtrees.every(subtree => subtree.hasCurrent())) {
    ...
    if (browserSubtrees.every(s => s.key() == browserSubtrees[0].key())) {
      ...
      const subtreeScores = walkTrees(
          browserSubtrees.map(s => s.value()), newPath, perDirScores, testFilter);
      if (isTopLevel(newPath)) {
        perDirScores.set(newPath, subtreeScores);
      }
...
function scoreBrowserSpecificFailures(runs, expectedBrowsers, {testFilter = null} = {}) {
  ...
  const perDirScores = new Map();
  const scores = walkTrees(runs.map(run => run.tree), '', perDirScores, testFilter);
  return [new Map(scores.map((score, i) => [runs[i].browser_name, score])), perDirScores];
}
